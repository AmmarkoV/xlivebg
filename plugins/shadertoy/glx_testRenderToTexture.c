...
int doDrawing()
{
   fprintf(stderr," doDrawing \n");
	...
	struct shaderObject * sho = loadShader("simple.vert", "simple.frag");
	if (sho==0) {  checkOpenGLError(__FILE__, __LINE__); exit(1); }

	struct shaderObject * textureFramebuffer = loadShader("virtualFramebuffer.vert", "virtualFramebufferSea.frag");
